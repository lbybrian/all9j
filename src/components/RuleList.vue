  ...
  <div>
    ...
    <div class="list">
      <el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.5)" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%">
        ...
        <el-table-column prop="name" label="规则名称"width="340" showOverflowTooltip>
        ...
        <el-table-column prop="ruleDesc" label="规则描述" showOverflowTooltip>
          <template slot-scope="scope">
          <div class="tableTitle ellipsis">
          	<p style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis!important;">
          		{{scope.row.ruleDesc}}</p>
          </div></template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="240">
          <template slot-scope="scope">
            ...
            <el-button @click="changeRule2(scope.row)" type="text" size="small">修改</el-button>
    ...
    <el-dialog v-if="checkVisible" :visible.sync="checkVisible" width="80%">
      ...
      <rule-detail :detail="detail" :startRule="startRule":stopRule="stopRule" :alarmRank="alarmRank" :ruleStatus="ruleStatus" :dict="dict" :ruleType="ruleType" :cycleSign="cycleSign"></rule-detail>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="checkVisible = false">关 闭</el-button>
      </span>
    </el-dialog>
    <!--修改弹框-->
    <el-dialog v-if="changeSign" :visible.sync="changeSign" width="80%">
      <span slot="title">规则修改</span>
      <!--<change-rule :detail="detail"  ref="changeRule" @closeChangeRule="closeChangeRule"></change-rule>-->
      <change-rule  :detail="gg"  ref="changeRule" v-bind="$props" @closeChangeRule="closeChangeRule" @changeRule="changeRule" ></change-rule>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="changeSign = false">关 闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  const API = '/api'
    ...
    created() {
        this.getList(1, 10, {})
//this.$Bus.$on('alarmRule', data => {
//      this.gg = data
//  	console.log('饼chartList-Table',this.tableData)
//    })
    },
    watch: {
      obj: {
...
//        console.log('zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz',this.detail)
//      },
////      status,
//    deep: true,
//    immediate: true,
...
//  	console.log(this.tableData)
        ...
        .catch(e => { // 请求出错
          this.loading = false
          this.$message({
            message: '请求列表失败woooooooooooooo：' + e,
            type: 'warning'
          })
        })
      },
...
//            this.getList(1, 10, {})
      ...
      // 结果
      resultRule(row) {
      	console.log('查看row。type',row.type)
        if (row.type === '1') { // 审计
          this.$router.push({
            path: '/auditResult',
            query: {
