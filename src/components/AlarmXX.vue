<template>
  <div class="alarmResult">
    <el-page-header @back="goBack" content="告警信息列表"></el-page-header>
    <!--<alarm-list :obj="params" v-if="params.status!==''"></alarm-list>-->
    ...
    <div class="list">
      <el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.5)" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%">
        ...
        <el-table-column prop="name" label="告警名称" width="340" showOverflowTooltip>
        ...
        <el-table-column prop="ruleName" label="规则名称" showOverflowTooltip>
          <template slot-scope="scope">
          <div class="tableTitle ellipsis">
          	<p style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis!important;">
          		{{scope.row.ruleName}}</p>
          </div>
          </template>
...
<script>
  ...
  export default {
    ...
    created() {
//  	console.log(1111111111111111,this.$route.query.id)
//    this.params.ruleId = this.$route.query.id;
      this.params.status = this.$route.query.status;
      this.getList(1, 10, this.params)
    },
    mounted() {
      if(this.backSign){
	      this.Back=this.backSign;
      }
    },
    methods: {
      goBack() {
