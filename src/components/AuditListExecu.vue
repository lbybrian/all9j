<template>
  <div class="excuebox" v-loading="loading">
    <!--<div v-if="tableData.length===0&&chartList.length===0&&this.dict===''&&objexe===''&&jsId===''">暂无数据</div>-->
    <div v-if="tableData.length===0&&chartList.length===0" v-loading="loading">暂无数据</div>
    <el-row v-show="chartList.length>0">
    ...
    <el-table v-loading="loading2" v-if="tableData.length>0" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.5)" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @row-click="rowModelHandel" :highlight-current-row='true' :row-class-name="tableRowIndex">
      <el-table-column type="index" :index="indexMethod" label="序号" width="80">
      </el-table-column>
      <el-table-column prop="softwareSign" label="软件名称"width="340">
        <template slot-scope="scope">
          <div class="tableTitle ellipsis"><p style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis!important;">{{softName[scope.row.softwareSign]}}</p>
          </div></template>
      </el-table-column>
      <el-table-column prop="moduleSign" label="模块名称" >
        <template slot-scope="scope">
        	<div class="tableTitle ellipsis"><p style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis!important;">{{blockName[scope.row.moduleSign]}}</p></div></template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" width="100">
          <template slot-scope="scope">
            <el-button @click="checkInstant(scope.row)" type="text" size="small">查看</el-button>
          </template>
        </el-table-column>
    </el-table>
...
<script>
  ...
  const API = '/api'
	export default {
		name: "AuditListExecu",
    props: ['jsId','objexe'],//audit,auresult
    ...
    data() {
        ...
        params: {},
        // 数据字典
        dict: JSON.parse(localStorage.getItem('dict'))?JSON.parse(localStorage.getItem('dict')):'暂无数据',
        // 软件名称模块名下拉框联动选项
//      options: '',
...
//    },
    ...
    created() {
      if (this.objexe) {
//    	console.log("AuditListExecu创建+objexe")
        this.params.ruleId = this.objexe
        this.getList(this.currentPage2, this.pageSize2, this.params)
      }
      else if(this.jsId){
//    	console.log("AuditListExecu创建+jsId")
      	this.params.ruleId=this.jsId
      	this.getList(this.currentPage2, this.pageSize2, this.params)
      }
      else{
//  	console.log(this.jsId,"创建时及时页面jsId和params.id",this.params.id,'图标',this.chartList.length)
      	this.getList(this.currentPage2, this.pageSize2, this.params)
      }
    },
    mounted() {},
    ...
    setRule(dict) {
      ...
      this.$store.state.rule.timeRangeSign = timeRangeSign
    	...
    	rowModelHandel(row,col,event){
    		this.rowIndex=row.index
    		this.getListChannel()
    	},
    	getListChannel(){
    		this.$nextTick(()=>{
    			this.$refs.multipleTable.setCurrentRow(this.templateList[this.rowIndex],true)
    		})
    	},
      ...
      ecportFun() {
        ...
        let url = API + '/exportLogData/exportExecuteRuleData?id=' + this.jsId
        location.href = url
      },
...
//      let url = params.url || '/analyzeRule/executeAnalyzeRuleById'//jk6
        let url = '/analyzeRule/executeAnalyzeRuleById'//jk6
//      console.log('谁的ID',params)
          ...
          if (d.chartType === 1) { // 饼图
            ...
            for (let i = 0; i< d.noPieDataMap.yDataList.length; i++) {
              series.push({
                ...
                name: d.noPieDataMap.yDataList[i].name,
              })
            }
