  ...
  <div class="all">
    <el-row>
      <el-col :offset="6" :span="14" class="center">
        <el-form>
          ...
          <!--<el-card >
            <span style="float: left;">统计类型:</span><br />-->
          	<!--<el-card class="box-card" v-if="detail.cycleSign==='1'&&detail.cycleStatTastDetail">
                <div slot="header" class="clearfix">
                  <span>周期统计数据表</span>
                </div>
                <el-form>
                  <el-form-item label="周期单位：">
                    {{this.tData.cycleStatTastDetail.cycleUnit?this.tData.cycleStatTastDetail.cycleUnit:'暂无数据'}}
                  </el-form-item>
                </el-form>
              </el-card>-->
            <!--<el-form-item label="周期统计数据表：" v-if="detail.cycleSign==='1'&&detail.cycleStatTastDetail">-->
            <el-form-item label="周期统计数据表：" v-if="detail.cycleStatTastDetail">
          ...
          <el-card class="box-card" v-else-if="detail.cycleSign==='2'&&detail.cycleStatTastDetail===null">
            <div slot="header" class="clearfix">
                  <span>及时统计数据</span>
                </div>
          ...
          <!-- 分桶信息表 -->
          <el-card class="box-card" v-if="detail.queryRuleJson.bucketBO.bucketName">
            <div slot="header" class="clearfix">
              <span>分组信息</span>
            </div>
            <el-form>
              ...
              <el-card class="box-card" v-for="item in detail.queryRuleJson.bucketBO.aggregationBOList">
                ...
                <el-form>
                  ...
                  <el-form-item label="聚合标识：" v-for="dcx in dict['013'].dataValue" v-if="item.aggSign===dcx.value">
                    ...
                    {{dcx.label}}
                  </el-form-item><el-form-item label="聚合别名：">
                  <!--</el-form-item><el-form-item label="聚合别名：" v-for="dcx in dict['013'].dataValue" v-if="item.aggAlias">-->
          ...
          <!-- 桶外聚合信息 -->
          <el-card class="box-card" v-if="detail.queryRuleJson.aggOutList.length>0">
            <!-- 空数据不显示 -->
            <div slot="header" class="clearfix" >
              <span>组外聚合</span>
            </div>
        ...
        <!-- 异常告警规则详情 -->
        <el-card v-if="detail.aardList.length>0">
        	
          <!--<span style="float: left;">异常告警规则详情:</span><br />-->
          <el-card class="box-card" v-for="(item,index) in detail.aardList" v-if="item.status==='1'" >
            <div slot="header" class="clearfix" >
              <!--<span>异常告警规则详情单{{++index}}</span><br />-->
              <span>异常告警规则详情单</span><br />
            </div>`
              <el-form>
                <el-form-item label="规则名称：">
        ...
        <el-card class="box-card " v-if="detail.arctList.length>0" >
          ...
          <el-form  v-for="item in detail.arctList" v-if="item.status==='1'">
